...
for i in images.dict.keys():
    print("processing: ", i)
    b,g,r=images.GetImage(i)
    
    vessle_analysis=IdentifyVascularNuclei(g,b)
    # vessle_analysis.get_nuclei_mask(nuc_mask)
    # vessle_analysis.find_vascular_nuclei()
    vascular, extra = vessle_analysis.get_sorted_masks()
    print("Found ", ndi.label(vascular)[1], " vascular associated nuclei")
    print("Found ", ndi.label(extra)[1], " extravascular nuclei")
    # vessle_analysis.print_masks()
    # vessle_analysis.show_status()
    
    
    output=ColocAnalysis(vascular, r).measure(nuclei_stats,print=False)
    df=pd.DataFrame(output).T.reset_index()
    df['loc_status']='Vascular'
    df['sample']=i
    output=ColocAnalysis(extra, r).measure(nuclei_stats,print=False)
    df2=pd.DataFrame(output).T
    
    
    with pd.ExcelWriter(f'../AnalysisOutput/{i}.thr_5750.xlsx') as writer: 
        df.to_excel(writer, sheet_name='vascular', index=False)
        df2.to_excel(writer, sheet_name='extravascular', index=False)
        

datasets=[df for df in os.listdir('../AnalysisOutput/') if '.xlsx' in df]        
results={}
for i in range(len(datasets)):
    results[datasets[i]]={}
    file=datasets[i]
    vas=pd.read_excel('../AnalysisOutput/'+file, sheet_name='vascular')
    ext=pd.read_excel('../AnalysisOutput/'+file, sheet_name='extravascular')
    
    results[datasets[i]]['pct_extravascular_cmv']=np.sum(ext['total_area'])/(np.sum(vas['total_area'])+np.sum(ext['total_area']))*100
    results[datasets[i]]['pct_transduced_vas']=len(vas[vas['total_area']>10])/len(vas)*100
    results[datasets[i]]['pct_transduced_extravas']=len(ext[ext['total_area']>10])/len(ext)*100
...
mean.to_csv('../CMV_localization_nuclear.csv', index=False)

highdose=mean[np.isin(mean['treatment'],['HighDose3weeks','WTUninjected'])]
highdose.columns=['Treatment','Region','Animal',
                  '% extravascular CMV signal',
                  '% transduced vascular cells',
                  '% transduced parenchymal cells']

sub=mean.drop('animal', axis=1)
mean_region=sub[sub['treatment']=='HighDose3weeks'].groupby(['region','treatment']).mean()

mean= pd.read_csv('../CMV_localization_nuclear.csv', index_col=None)
plt.figure(figsize=(3,6), dpi=300)
sns.barplot(data=mean[mean['treatment']!='WTUninjected'], x='treatment', y='pct_extravascular_cmv')
plt.xticks(rotation=45, ha='right')
plt.ylim((50,100))
plt.show()
sns.barplot(data=mean, x='treatment', y='pct_transduced_vas')
plt.show()
sns.barplot(data=mean, x='treatment', y='pct_transduced_extravas')
plt.show()
# plt.hist(np.log(df['total_area']+1))

df2=df[['treatment','pct_transduced_vas','pct_transduced_extravas']]
df2.columns=['treatment','vascular','extravascular']
df3=df2.melt(id_vars=['treatment'])

plt.figure(figsize=(3,6), dpi=300)
sns.barplot(data=df3, x='treatment', y='value', hue='variable')
plt.xticks(rotation=45, ha='right')
